...
1. Obtain a tenant slug and API key — see Authentication below.
2. Create an instance with `POST /v1/instances`, supplying an SSH public key.
3. Poll `GET /v1/instances/{name}` until `status` flips from `provisioning` to `ready`.
4. Connect with `ssh <hostname>` using the matching private key.
